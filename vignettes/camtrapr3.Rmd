---
title: "3. Extracting Data from Camera Trapping Images and videos, creating occupancy & secr input"
...
output:
  html_document:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{3. Extracting data from camera trap images, creating occupancy & secr input}
...
Here's how the daily aggregates are calculate for the different combinations of allCamsOn and camerasIndependent, and depending on whether input columns are date or date-time (from v2.1.0):

| camerasIndependent | allCamsOn | input columns  | aggregate with |
|---|---|---|---|
| TRUE | TRUE | date-time | ifelse(all(x == 1), sum(x), min(x) * length(x)) |
| TRUE | TRUE | date |  ifelse(all(x == 1), sum(x), 0) |
| TRUE | FALSE | date-time | sum  |
| TRUE | FALSE | date| sum |
| FALSE |TRUE | date-time | min |
| FALSE| TRUE | date | ifelse(all(x == 1), 1, 0) |
| FALSE| FALSE | date-time | mean |
| FALSE |FALSE | date| ifelse(sum(x, na.rm = TRUE) >= 1, 1, sum(x, na.rm = TRUE))  |
