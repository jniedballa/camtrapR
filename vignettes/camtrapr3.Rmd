...
output:
  ...
  html_document: null
vignette: |
  %\VignetteIndexEntry{3. Data Extraction from Images and videos, creating occupancy & secr input} \usepackage[utf8]{inputenc}
---
...
As you can see, allCamsOn has no effect if all cameras were active (due to hasProblems = FALSE). The differences on the setup/retrieval day are due to camerasIndependent.
...
There is a little function for plotting the camera operation matrix included in the package, accessible directly via plot(). Imagine there is a typo in one of your date fields and the setup or retrieval year is wrong. You will easily be able to spot it this way.
...
par(mfrow = c(1,2))
plot(camop_problem)
plot(camop_problem, palette = "Heat")
```
...
```{r}
plot(camop_problem, lattice = TRUE)
plot(camop_problem, palette = "Heat", lattice = TRUE)
```
