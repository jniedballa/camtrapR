...
col3 = c("detection submodel", "occupancy submodel", "denotes the intercepts (alpha0, beta0)",
         "fixed effects (constant across species)", "independent effects (separate for each species)", "random effects (of species and/or other categorical covariates)",
...
In contrast to single-species occupancy models in unmarked and ubms, this community occupancy workflow does not allow NAs in the detection histories due to technical differences in the implementation. The detection histories may only contain 1 and 0 for community occupancy models.

Information about occasions without effort is provided via the mandatory observation covariate effort. Use the `$effort` part of the output of `detectionHistory()`. All occasions with NA in the effort matrix will be set to detection probability = 0. This ensures that those occasions have detection probability = 0 and do not information to the model fit. 

Code for converting NAs in the detection history to 0:

```{r}
# replace NAs with 0 in the ylist component of input_AHM (for all species at once)
input_AHM$ylist <- lapply(input_AHM$ylist, FUN = function(x) {
  x[is.na(x)] <- 0
  x
})
```
...
It is not relevant in this example because the detection histories contain no NAs, but it is a common issue in real data. 
